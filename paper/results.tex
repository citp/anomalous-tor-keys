...
has been hosted on a MIPS machine which might hint to a small embedded device
with a weak random number generator:
...
To further investigate, we checked whether
``DesasterBlaster`` shares prime factors with any other relays, but it seems
like the relay has rotated multiple identity keys and it only shares prime
factors between its own keys.
...
An onion service's position in the hash ring is governed by
