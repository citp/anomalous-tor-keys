...
\end{tikzpicture}
\caption{Each day, onion services are placed at a different position in Tor's
``hash ring'' which is a list of HSDirs, sorted by their fingerprint.  The three
immediate HSDir neighbors in positive direction constitute the first replica
while the second replica is at another, pseudorandom position.}
\label{fig:hash-ring}
\end{figure}
...
fingerprint in ascending order, the three HSDirs (for both replicas) whose
fingerprints are the closest to the descriptor in the positive direction are the
onion service's HSDirs, as shown in Figure~\ref{fig:hash-ring}.  This set of six
HSDirs changes each day when \textit{time-period} increments.
